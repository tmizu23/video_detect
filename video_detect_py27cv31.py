        ...
        if self.curpos > 2 or self.webcam:
            ...
            if fgmask is not None:
                ...
                if 0 < len(cnts) and (self.curpos > 3 or self.webcam):
                    ...
                    if self.state == "DETECT" and self.tracking is False:
                        print cv2.arcLength(cnt_max,True)
                        print x_max, y_max, w_max, h_max
                        self.track_window = (x_max, y_max, w_max, h_max)
                        roi = gframe[y_max:y_max+h_max, x_max:x_max+w_max]
                        self.roi_hist = cv2.calcHist([roi],[0],None,[255],[0,255])
    ...
    def writeout_video(self):
        ...
        if self.state == "DETECT" or self.writing:
            ...
            if self.writing:
                ...
                self.videoWriter = cv2.VideoWriter(outfile, cv2.FOURCC(
