...
Examples:
    video_detect.py --inpdir=F://video --outdir=F://capture -w -b -j- -area=0,680,100,1280
...
class Video():
    ...
    def __init__(self, playfile, outdir, webcam=False):
        ...
        if webcam:
            ...
            self.learningrate = 1.0/90  # 背景差分の更新頻度
            self.cap = cv2.VideoCapture(0)
        else:
            self.webcam = False
            self.playfile = playfile.encode(sys.getfilesystemencoding())
            self.learningrate = 1.0/150 #1.0/90 # 0だとだめ
            self.cap = cv2.VideoCapture(self.playfile)
        self.outdir = outdir.encode(sys.getfilesystemencoding())
        ...
        self.verbose = True
        # 背景差分動体
        self.fgbg = cv2.BackgroundSubtractorMOG()  # 混合正規分布
        ...
        self.frame_length_after_detection = 60  # 検知後の動画を保存するフレーム数
        ...
        self.predict_animal = True  # 機会学習による予測をするかどうか
    ...
    def writeout_video(self):
        ...
        if self.state == "DETECT" or self.writing:
            ...
            if self.writing:
                ...
                if self.webcam:
                    filename = "webcam" + datetime.datetime.now().strftime('%Y-%m-%d_%H_%M_%S') + '.m4v'
                else:
                    filename = splitext(basename(self.playfile))[
                        0] + '_' + str(self.cursec).zfill(5) + '.mov'
                outfile = join(self.outdir, filename)
                self.videoWriter = cv2.VideoWriter(outfile, cv2.cv.CV_FOURCC(
    ...
    def writeout_result(self, outfile):
        ...
        if not os.path.exists(outfile):
            ...
            f = open(outfile, 'w')
            f.write(datastr.encode(sys.getfilesystemencoding()) + '\n')
            f.close()
        ...
        f = open(outfile, 'a')
        # f.write(datastr.encode('shift-jis') + '\n')
        f.write(datastr + '\n')
        f.close()
        logstr = datastr.replace(',', '\t') + '\n'
        logstr = unicode(logstr, encoding=sys.getfilesystemencoding())
        self.notify(logstr)
    ...
    def init_learning(self):
        u"""背景画像の学習."""
        if not self.webcam:
            self.cap.set(cv2.cv.CV_CAP_PROP_POS_FRAMES, self.framecount - 51)
            ...
            for i in range(50):
                ...
                # gframe = cv2.GaussianBlur(gframe, (41, 41), 0)  # 映像ノイズ削除
                gframe = cv2.equalizeHist(gframe)  # ヒストグラム平坦化（明るさの変化を抑えるため）
                self.fgbg.apply(gframe, learningRate=0.02)
            self.cap.set(cv2.cv.CV_CAP_PROP_POS_FRAMES, self.curpos)

    def capture_nextframe(self):
        u"""ビデオフレーム読み込み＆処理."""
        fgmask = None
        self.cursec = int(self.cap.get(cv2.cv.CV_CAP_PROP_POS_MSEC) / 1000)
        self.curpos = int(self.cap.get(cv2.cv.CV_CAP_PROP_POS_FRAMES))
        ...
        if self.curpos > 2 or self.webcam:
            ...
            if fgmask is not None:
                ...
                fgmask = cv2.morphologyEx(fgmask, cv2.MORPH_OPEN, kernel)
                gframe = cv2.bitwise_and(gframe, gframe, mask=fgmask)
                # メディアンフィルタでノイズ除去
                ...
                if 0 < len(cnts) and (self.curpos > 3 or self.webcam):
                    ...
                    if self.bounding:
                        for c in cnts:
                            x, y, w, h = cv2.boundingRect(c)+ np.array([self.detect_left,self.detect_top,0,0])
        ...
        #               self.detect_left:self.detect_right] = gframe
...
class Settei():
    ...
    def load_cui_settings(self):
        ...
        if args["--inpdir"]:
            inpdir = unicode(args["--inpdir"], encoding=sys.getfilesystemencoding())
            self.settings["playdir"] = inpdir
        if args["--outdir"]:
            outdir = unicode(args["--outdir"], encoding=sys.getfilesystemencoding())
    ...
    def load_settings(self):
        ...
        if os.path.exists(setting_file):
            f = open(setting_file, 'r')  # 書き込みモードで開く
...
class MainView(QtGui.QMainWindow, Ui_MainWindow):
    ...
    def get_detectionarea(self):
        ...
        top = self.detectionTop_Edit.text().toInt()[0]
        bottom = self.detectionBottom_Edit.text().toInt()[0]
        left = self.detectionLeft_Edit.text().toInt()[0]
        right = self.detectionRight_Edit.text().toInt()[0]
    ...
    def get_outputdata(self):
        u"""出力するデータの種類を設定."""
        video = self.writevideo_checkBox.isChecked()
        jpg = self.writejpg_checkBox.isChecked()
        graph = self.writegraph_checkBox.isChecked()
        return video, jpg, graph
    ...
    def get_outdir(self):
        ...
        outdir = unicode(outdir.toUtf8(), encoding="UTF-8")
    ...
    def get_playdir(self):
        ...
        playdir = QtGui.QFileDialog.getExistingDirectory(
                    self, "Select folder")
        playdir = unicode(playdir.toUtf8(), encoding="UTF-8")
        return playdir
    ...
    def get_playlist(self, playdir):
        ...
        for i in range(model.rowCount(idx)):
            ...
            if model.isDir(child):
                ...
                playlist.append(unicode(model.filePath(
                    child).toUtf8(), encoding="UTF-8").replace('/', os.sep))
    ...
    def get_filename_treeview(self, index):
        ...
        filename = unicode(model.filePath(indexItem).toUtf8(), encoding="UTF-8").replace('/', os.sep)
        return filename
...
class MainController():
    ...
    def set_actions(self):
        ...
        self.main_view.writevideo_checkBox.stateChanged.connect(self.set_outputdata)
        self.main_view.writejpg_checkBox.stateChanged.connect(self.set_outputdata)
        self.main_view.writegraph_checkBox.stateChanged.connect(self.set_outputdata)
    ...
    def set_outputdata(self):
        u"""出力するデータの種類を設定."""
        print "main_ctr:set_outputdata"
        video, jpg, graph = self.main_view.get_outputdata()
        self.settei.settings["writevideo"] = video
        self.settei.settings["writejpg"] = jpg
        self.settei.settings["writegraph"] = graph
...
class CuiController():
        ...
        if os.path.exists(playdir):
            ...
            if len(self.playlist) > 0:
                ...
                for playfile in self.playlist:
                    # 再生ビデオの初期設定.
                    print(playfile)
                    tmppath = self.settei.settings["outdir"] + os.sep + playfile.replace(self.settei.settings["playdir"], "")
                    recursive_outdir = os.path.dirname(tmppath)
                    self.playfile = playfile
                    self.video = Video(playfile, recursive_outdir)
                    self.video.set_bounding(self.settei.settings["bounding"])
    ...
    def get_playlist(self, playdir):
        ...
        for file in self.fild_all_files(playdir):
            ...
            if self.settei.settings.has_key(ext) and self.settei.settings[ext]:
...
class App(QtGui.QApplication):
    ...
    def __del__(self):
        ...
        if self.main_ctrl.video is not None:
